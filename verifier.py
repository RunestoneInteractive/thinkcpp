...
class EvalTestNeedsCorrect(CppsXmlTest):
    """at least one test under an evaluate needs to be correct"""

    @classmethod
    def test_file(cls, fname, doc):
        ret = True
        for evaluation in doc.getElementsByTagName('evaluation'):
            for evaluate in evaluation.getElementsByTagName('evaluate'):
                hasTest = False
                hasCorrect = False
                for tst in evaluate.getElementsByTagName('test'):
                    hasTest = True
                    if tst.getAttribute("correct") == "yes":
                        hasCorrect = True
                if hasTest and not hasCorrect:
                    ret = False
                    print(f'{fname}: evaluation/evaluate/test has no correct answer')
        return ret


class FillInTests(CppsXmlTest):
    """fillin should not have case or mode attributes"""

    @classmethod
    def test_file(cls, fname, doc):
        ret = True
        for fillin in doc.getElementsByTagName('fillin'):
            if fillin.hasAttribute("mode"):
                print(f'{fname}: fillin has mode attribute')
                ret = False
            if fillin.hasAttribute("case"):
                print(f'{fname}: fillin has case attribute')
                ret = False
        return ret
